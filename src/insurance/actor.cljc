(ns insurance.actor
  "InsuranceBrokerageActor — the ISCO-08 3321 community insurance actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one insurance operation request
  ...
  The unconditional invariant: the InsuranceBrokerageAdvisor can never
  directly commit a record the InsuranceBrokerageGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [insurance.advisor :as advisor]
            [insurance.governor :as governor]
            [insurance.store :as store]))

(defn build-graph
  "Build a compiled InsuranceBrokerageActor graph. `store` implements
  `insurance.store/Store`. `advisor` implements
  `insurance.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :application-id (:application-id proposal)
                                    :payload proposal}]
